...
public class CoyoteAdapter implements Adapter {
	...
	private static final String POWERED_BY = "Servlet/3.0 JSP/2.2 " + "(" + ServerInfo.getServerInfo() + " Java/" + System.getProperty("java.vm.vendor") + "/"
	...
	public boolean event(org.apache.coyote.Request req, org.apache.coyote.Response res, SocketStatus status) {
		...
		try {
			...
			if (response.isClosed() || !request.isComet()) {
				if (status == SocketStatus.OPEN && request.getEvent().getEventType() != EventType.END) {
	...
	protected boolean postParseRequest(org.apache.coyote.Request req, Request request, org.apache.coyote.Response res, Response response) throws Exception {
		...
		while (mapRequired) {
			...
			if (request.getServletContext().getEffectiveSessionTrackingModes().contains(SessionTrackingMode.URL)) {
				...
				sessionID = request.getPathParameter(ApplicationSessionCookieConfig.getSessionUriParamName(request.getContext()));
				if (sessionID != null) {
