...
public class Http11Processor extends AbstractHttp11Processor<Socket> {
	...
	public SocketState process(SocketWrapper<Socket> socketWrapper) throws IOException {
		...
		// sensible result
		if (endpoint.getCurrentThreadsBusy() > 0 && endpoint.getMaxThreads() > 0) {
			threadRatio = (endpoint.getCurrentThreadsBusy() * 100) / endpoint.getMaxThreads();
		}
		...
		while (!error && keepAlive && !endpoint.isPaused()) {
			...
			try {
				 ...
				 */
				if (inputBuffer.lastValid == 0 && socketWrapper.getLastAccess() > -1 && standardTimeout > 0) {
