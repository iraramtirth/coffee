...
class BluetoothEventLoop {
        ...
        public void handleMessage(Message msg) {
            ...
            case EVENT_AGENT_CANCEL:
                ...
                mBluetoothService.getBondState().setBondState(address,
                        ...
                        IBluetoothDevice.UNBOND_REASON_REMOTE_AUTH_CANCELED);
                break;
            }
        }
    };
    ...
    private void onDeviceDisappeared(String address) {
        Intent intent = new Intent(IBluetoothDevice.ACTION_DISAPPEARED);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mAdapter.getRemoteDevice(address));
        mContext.sendBroadcast(intent, BLUETOOTH_PERM);
    }
    ...
    private void onCreatePairedDeviceResult(String address, int result) {
        ...
        if (result == IBluetoothDevice.BOND_SUCCESS) {
        ...
        } else if (result == IBluetoothDevice.UNBOND_REASON_AUTH_FAILED &&
                mBluetoothService.getBondState().getAttempt(address) == 1) {
            mBluetoothService.getBondState().addAutoPairingFailure(address);
            pairingAttempt(address, result);
        } else if (result == IBluetoothDevice.UNBOND_REASON_REMOTE_DEVICE_DOWN &&
    ...
    private void onDeviceRemoved(String deviceObjectPath) {
        ...
        if (address != null) {
            mBluetoothService.getBondState().setBondState(address.toUpperCase(),
                    BluetoothDevice.BOND_NONE, IBluetoothDevice.UNBOND_REASON_REMOVED);
...
//                intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
    ...
    private void onRequestPairingConsent(String objectPath, int nativeData) {
        ...
        Intent intent = new Intent(IBluetoothDevice.ACTION_PAIRING_REQUEST);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mAdapter.getRemoteDevice(address));
        intent.putExtra(IBluetoothDevice.EXTRA_PAIRING_VARIANT,
        		IBluetoothDevice.PAIRING_VARIANT_CONSENT);
    ...
    private void onRequestPasskeyConfirmation(String objectPath, int passkey, int nativeData) {
        ...
        Intent intent = new Intent(IBluetoothDevice.ACTION_PAIRING_REQUEST);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mAdapter.getRemoteDevice(address));
        intent.putExtra(IBluetoothDevice.EXTRA_PASSKEY, passkey);
        intent.putExtra(IBluetoothDevice.EXTRA_PAIRING_VARIANT,
        		IBluetoothDevice.PAIRING_VARIANT_PASSKEY_CONFIRMATION);
        mContext.sendBroadcast(intent, BLUETOOTH_ADMIN_PERM);
        return;
    }
    ...
    private void onRequestPasskey(String objectPath, int nativeData) {
        ...
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mAdapter.getRemoteDevice(address));
        intent.putExtra(IBluetoothDevice.EXTRA_PAIRING_VARIANT,
        		IBluetoothDevice.PAIRING_VARIANT_PASSKEY);
        mContext.sendBroadcast(intent, BLUETOOTH_ADMIN_PERM);
        return;
    }
    ...
    private void onRequestPinCode(String objectPath, int nativeData) {
        ...
        if (address.equals(pendingOutgoingAddress)) {
            ...
            if (mBluetoothService.isBluetoothDock(address)) {
                ...
                mBluetoothService.setPin(address,
//                		BluetoothDevice.convertPinToBytes(pin));
                	(byte[])Exe.run(BluetoothDevice.class, "convertPinToBytes", new Object[]{pin}));
                return;
            }
...
//                    		BluetoothDevice.convertPinToBytes("0000"));
                    (byte[])Exe.run(BluetoothDevice.class, "convertPinToBytes", new Object[]{"0000"}));
                    return;
                }
           }
        }
        Intent intent = new Intent(IBluetoothDevice.ACTION_PAIRING_REQUEST);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mAdapter.getRemoteDevice(address));
        intent.putExtra(IBluetoothDevice.EXTRA_PAIRING_VARIANT, IBluetoothDevice.PAIRING_VARIANT_PIN);
        mContext.sendBroadcast(intent, BLUETOOTH_ADMIN_PERM);
        return;
    }
    ...
    private void onDisplayPasskey(String objectPath, int passkey, int nativeData) {
        ...
        intent.putExtra(IBluetoothDevice.EXTRA_PASSKEY, passkey);
        intent.putExtra(IBluetoothDevice.EXTRA_PAIRING_VARIANT,
        		IBluetoothDevice.PAIRING_VARIANT_DISPLAY_PASSKEY);
        mContext.sendBroadcast(intent, BLUETOOTH_ADMIN_PERM);
    }
    ...
    private void onAgentCancel() {
        Intent intent = new Intent(IBluetoothDevice.ACTION_PAIRING_CANCEL);
        mContext.sendBroadcast(intent, BLUETOOTH_ADMIN_PERM);
