...
import android.media.IAudioManager;
...
public class BluetoothA2dpService extends IBluetoothA2dp.Stub {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(IAudioManager.VOLUME_CHANGED_ACTION)) {
                int streamType = intent.getIntExtra(IAudioManager.EXTRA_VOLUME_STREAM_TYPE, -1);
                if (streamType == AudioManager.STREAM_MUSIC) {
                    BluetoothDevice sinks[] = getConnectedSinks();
                    if (sinks.length != 0 && isPhoneDocked(sinks[0])) {
                        String address = sinks[0].getAddress();
                        int newVolLevel =
                          intent.getIntExtra(IAudioManager.EXTRA_VOLUME_STREAM_VALUE, 0);
                        int oldVolLevel = 0;
//                          intent.getIntExtra(AudioManager.EXTRA_PREV_VOLUME_STREAM_VALUE, 0);
    ...
    public BluetoothA2dpService(Context context, BluetoothService bluetoothService) {
        ...
        mIntentFilter.addAction(IAudioManager.VOLUME_CHANGED_ACTION);
...
//                Settings.Secure.getBluetoothA2dpSinkPriorityKey(device.getAddress()),
                Exe.run(Settings.Secure.class, "getBluetoothA2dpSinkPriorityKey", new Object[]{device.getAddress()}) + "",
