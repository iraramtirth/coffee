...
public class MainTabActivity extends TabActivity
			implements IActivity, OnTabChangeListener {
	...
	private boolean isFirstLogin(){
		SharedPreferences prefs = getSharedPreferences("mmb", MODE_WORLD_WRITEABLE);
		boolean isFirst = prefs.getBoolean(IActivity.PREF_IS_FIRST_LOGIN, true);
		if(isFirst == true){
			prefs.edit().putBoolean(IActivity.PREF_IS_FIRST_LOGIN, false).commit();
			return true;
		}
		return false;
	}
	...
	private void createTab(){
		...
		intent.putExtra(ACTIVITY_CLASS_NAME, SaleNowActivity.class.getName());
		//intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		tabHost.addTab(tabHost.newTabSpec(tabs[0])
				.setContent(intent)
				.setIndicator("",this.getResources().getDrawable(R.drawable.tab_00)));
		//下期抢购
		intent = new Intent(this, TabFlipperActivity .class);
		intent.putExtra(ACTIVITY_CLASS_NAME, SaleNextActivity.class.getName());
		//intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		tabHost.addTab(tabHost.newTabSpec(tabs[1])
				.setContent(intent)
				.setIndicator("",this.getResources().getDrawable(R.drawable.tab_10)));
		//我的订单
		intent = new Intent(this, TabFlipperActivity.class);
		intent.putExtra(ACTIVITY_CLASS_NAME, OrderHomeActivity.class.getName());
		intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		tabHost.addTab(tabHost.newTabSpec(tabs[2])
				.setContent(intent)
				.setIndicator("",this.getResources().getDrawable(R.drawable.tab_20)));
		//更多
		intent = new Intent(this, TabFlipperActivity.class);
		intent.putExtra(ACTIVITY_CLASS_NAME, HelpActivity.class.getName());
		intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		tabHost.addTab(tabHost.newTabSpec(tabs[3])
				.setContent(intent)
				.setIndicator("",this.getResources().getDrawable(R.drawable.tab_30)));
	}
	...
	private void setTabStyle(){
		try {
			...
			LinearLayout.LayoutParams lLayoutParams = new LinearLayout.LayoutParams(-1, -2);
			lLayoutParams.weight = 1;
			for(int i=0; i < tabs.length; i++){
				final ImageButton tab = (ImageButton) View.inflate(this, R.layout.tab, null);
				int resid = R.drawable.class.getField(TAB_PRE + i + TAB_NOT_SELECTED).getInt(null);
				if(i == 0){
					resid = R.drawable.class.getField(TAB_PRE + i + TAB_SELECTED).getInt(null);
				}
	...
	public void onTabChanged(String tabId) {
		...
		try {
			...
			int resid = R.drawable.class.getField(tabId + TAB_SELECTED).getInt(null);
			//帮助：显示菜单
			if(popMenu != null){
	...
	private void createPopMenu(){
		...
		if(popMenu == null){
			...
			for(int i=0; i<4; i++){
				final int resid = RUtils.getResId(this.getPackageName() + ".R$id", "menu_item_"+i);
...
//							intent.setClass(context, MmbBrowserActivity.class);
//							intent.putExtra(IActivity.KEY_WEBVIEW_URL, SysConfig.HELP);
//							startActivityForResult(intent, SysConfig.REQUEST_CODE_MMBSITE);
						...
						case 1://版本
							try {
								...
								drawableResid = new Integer[]{
										null,
										R.drawable.alert_dialog_sure_selector,
										null};
								clickListeners = new View.OnClickListener[]{null ,null, null};
								Alert.show(context, title, new String[]{info.versionName}, clickListeners, drawableResid);
						...
						case 2: //mmb主站
							intent = new Intent();
//							intent.setClass(context, MmbBrowserActivity.class);
//							intent.putExtra(IActivity.KEY_WEBVIEW_URL, SysConfig.MMB_HOST);
//							startActivityForResult(intent, SysConfig.REQUEST_CODE_MMBSITE);
							intent.setAction(Intent.ACTION_VIEW);
