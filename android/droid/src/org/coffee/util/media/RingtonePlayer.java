...
public class RingtonePlayer {
	...
	public void stop() {
		try {
			if (null != mMediaPlayer && mMediaPlayer.isPlaying()) {
				mMediaPlayer.stop();
				Log.i(TAG, "VOIP_MediaPlayer_stop");
			}
	...
	private void createMediaPlayer() {
		try {
			...
			case RING_CALL_OUT:
				afd = this.mContext.getResources().openRawResourceFd(R.raw.voip_call_ring);
			case RING_CALL_BUSY:
				if (null == afd) {
					afd = this.mContext.getResources().openRawResourceFd(R.raw.voip_busy);
				}
				// 摩托Me525 需要通过该方式设置听筒播放
				// if (false)
				{
					mMediaPlayer.setAudioStreamType(AudioManager.FLAG_PLAY_SOUND);
				}
		...
		public void run() {
			try {
				...
				if (null == mMediaPlayer || !mMediaPlayer.isPlaying()) {
					...
					mMediaPlayer.start();
					Log.i(TAG, "VOIP_MediaPlayer_start");
				}
