...
public class DbHelper extends SQLiteOpenHelper {
	...
	public DbHelper(Context context) {
		...
		synchronized (this) {
			...
			if (!db.isOpen()) {
				this.db.openDatabase(App.getContext().getDatabasePath(App.DB_NAME).getPath(), null, SQLiteDatabase.OPEN_READWRITE);
			}
		}
	}

	/**
	 * {@link #getWritableDatabase()} Create and/or open a database that will be
	 * used for reading and writing. 数据库创建， onCreate执行， 注意该方法只执行一次
	 */
	@Override
